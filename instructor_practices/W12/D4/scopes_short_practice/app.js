...
app.get('/instruments/keyboard', async (req, res, next) => {
    const keyboards = await Instrument.scope({method: ['isType', 'keyboard']}, 'defaultScope').findAll()
    res.json(keyboards);
});


app.get('/instruments/string', async (req, res, next) => {
    const strings = await Instrument.scope({method: ['isType', 'string']}, 'defaultScope').findAll()
    res.json(strings);
});

app.get('/instruments/woodwind', async (req, res, next) => {
    const woodWinds = await Instrument.scope({method: ['isType', 'woodwind']}, 'defaultScope').findAll()
    res.json(woodWinds);
});
...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
    ...
    const filterStoreInstruments = await Instrument.scope({method: ['storeIdOrderByName', storeId]}, 'defaultScope').findAll()
    // Your code here
